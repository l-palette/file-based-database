...
class HomeLibrary:
    ...
    def search_books(self, field: str, value: str) -> List[str]:
        return [
            book.name for book in self.books if value in str(getattr(book, field, ""))
        ]
    ...
    def save_books_to_file(self):
        with open(FILENAME, "w", encoding="utf-8") as file:
            for book in self.books:
                file.write(
                    f"{book.name}|{book.year}|{book.authors}|{book.genres}|{book.width}|{book.height}|{book.book_type}|{book.source}|{book.date_added}|{book.date_read}|{book.rating}\n"
                )
    ...
    while True:
        ...
        if choice == "1":
            try:
                ...
                authors = get_input(
                    "Введите авторов (через запятую): ", "authors", validate_authors
                )
                genres = get_input(
                    "Введите жанры (через запятую): ", "genres", validate_genres
                )
                year = int(get_input("Введите год издания: ", "year", validate_year))
                width = float(
                    get_input(
                        "Введите ширину книги (в мм, например 200.0): ",
                        "width",
                        validate_width,
                    )
                )
                height = float(
                    get_input(
                        "Введите высоту книги (в мм, например 200.0): ",
                        "height",
                        validate_height,
                    )
                )
                book_type = get_input(
                    "Введите тип книги (мягкий/твердый): ",
                    "book_type",
                    validate_book_type,
                )
                source = get_input(
                    "Введите источник (покупка/подарок/наследство): ",
                    "source",
                    validate_source,
                )
                date_added = get_input(
                    "Введите дату добавления (дд-мм-гггг): ",
                    "date_added",
                    validate_date_added,
                    year,
                )
                date_read = get_input(
                    "Введите дату чтения (дд-мм-гггг, оставьте пустым, если не читали): ",
                    "date_read",
                    validate_date_read,
                    date_added,
                )
                rating = get_input(
                    "Введите оценку книги (n/10 - комментарий, оставьте пустым, если не читали): ",
                    "rating",
                    validate_rating,
                )
            ...
            field = get_input(
                "Введите поле для поиска (name/authors/genres): ",
                "search_field",
                validate_search_field,
            )
            value = get_input(
                "Введите значение для поиска: ", field, validation_functions[field]
            )
            ...
            field = get_input(
                "Введите поле для обновления (name/authors/genres/year/width/height/book_type/source/date_added/date_read/rating):",
                "update_field",
                validate_update_field,
            )
            new_value = get_input(
                "Введите новое значение: ", field, validation_functions[field]
            )
